...
public class HealingZone : NetworkBehaviour
{
    ...
    [Header("Settings")]
    [SerializeField] private int _maxHealPower = 15;
    [SerializeField] private float _cooldown = 60.0f, _healTickRate = 1.0f;
    ...
    private void Update()
    {
        ...
        if (_tickTimer >= _healTickRate)//힐 틱을 계싼할 차례가 온거야
        {
            foreach (PlayerController item in _playerInZone)
            {
                if (_healPower.Value <= 0) break;

                if (item.HealthCompo.currentHealth.Value == item.HealthCompo.maxHealth) continue;


                if (item.CoinCompo.totalCoin.Value <= _coinPerTick) continue;

                item.CoinCompo.SpendCoin(_coinPerTick);

                item.HealthCompo.RestoreHealth(_healPerTick);
